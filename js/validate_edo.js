...
let validateNumber_b = () =>{
	let b = document.getElementById('xn').value;
	if (!nF.test(b)) {
		return false;
	}
	return true;
...
let validateNumber_c = () =>{
	let c = document.getElementById('y0').value;
	if (!nF.test(c)) {
		return false;
	}
	return true;
	...
	switch (name) {
		...
		case 'n':
			...
			if (result) {
				...
				document.getElementById('error4').innerHTML = "Número debe ser un entero positivo";
